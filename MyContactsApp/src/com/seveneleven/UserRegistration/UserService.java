...
public class UserService {
    ...
    public Optional<User> findUserByEmailAndPassword(String email, String password) {
        ...
        String hashedPassword = hashPassword(password);
        for (User user : users) {
            if (user.getEmail().equalsIgnoreCase(email) && user.getPasswordHash().equals(hashedPassword)) {
    ...
    private void validateEmail(String email) throws ValidationException {
        ...
        if (email == null || !email.matches(emailRegex)) throw new ValidationException("Invalid email format.");
    }
    ...
    private void validatePhone(String phone) throws ValidationException {
        if (phone == null || phone.trim().isEmpty()) throw new ValidationException("Phone cannot be empty.");
        if (!phone.matches("^[0-9]{7,15}$")) throw new ValidationException("Phone must be 7 to 15 digits.");
    }

    private void validateAddress(String address) throws ValidationException {
        if (address == null || address.trim().isEmpty()) throw new ValidationException("Address cannot be empty.");
    }
    ...
    private void checkDuplicateEmail(String email) throws ValidationException {
        for (User user : users) {
            if (user.getEmail().equalsIgnoreCase(email)) throw new ValidationException("Email already registered.");
        }
    }
